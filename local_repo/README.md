...
Мои впечателения
Почему фреймворк так себя ведет:

    Зоопарк технологий: Feast пытается подружить Python, 
SQL-базу (Postgres) и C++ движок (PyArrow). 
Ошибка ArrowNotImplementedError, с которой мы боролись, — 
это конфликт на стыке этих систем. Postgres отдал JSON, 
Python сделал из него словарь, а PyArrow «завис», не 
понимая, как это упаковать в строгий бинарный формат.
    «Contrib» статус: Поддержка PostgreSQL в Feast до сих 
пор находится в папке contrib (сообщество). Это не ядро 
системы (как BigQuery или Snowflake), поэтому там 
много «недопиленных» углов.
    Строгость вместо гибкости: Feature Store создавался 
для того, чтобы данные в обучении и в продакшене были 
идентичны на 100%. Поэтому он падает при малейшем 
несоответствии типов, вместо того чтобы тихо «привести 
